...
class Krea2AssistedUiTest(unittest.TestCase):
    ...
    def test_exposes_a_distinct_assisted_creation_mode(self):
        ...
        self.assertIn('<option value="5.0.0">V5 · templates KREA2', self.page)
        self.assertIn('id="krea2-assisted-example-variant"', self.page)
        self.assertIn("next.wildcard_library", self.script)
        self.assertIn('id="krea2-assisted-library-status"', self.page)
        self.assertIn('id="krea2-assisted-example-candidates"', self.page)
        self.assertIn('const defaultAssistedWorkflowId = "krea2-flux-klein@1.0.0";', self.script)
        self.assertIn('/cielbleukrea2_v1bf16/i', self.script)
        self.assertIn('else loadDefaultRenderSettings();', self.script)
        self.assertIn('project.style_preset) loadDefaultWorkflow();', self.script)
        self.assertIn('.krea2-assisted-example-candidate img', self.css)
