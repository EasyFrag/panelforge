    ...
    mount(prefix, { request, onChange }) {
      ...
      function error() {
        ...
        for (const entry of data.entries) {
          if (!entry.name || names.has(entry.name.toLowerCase())) return "Choisissez deux fichiers LoRA distincts.";
      ...
      function note() {
        ...
        if (element) element.textContent = loading ? "Lecture des LoRA sur ComfyUI…" : error() || warning || (
          perPass() ? "Ordre identique dans les deux passes. Chaque passe utilise ses propres forces." : "Les LoRA s’appliquent dans l’ordre des lignes.");
      }
      function controls() {
        root.querySelectorAll("input, select, button").forEach(el => {
          ...
          if (el.matches("[data-lora-add]")) el.disabled ||= data.entries.length >= 2 || !models.some(name => !data.entries.some(e => e.name === name));
      ...
      function add() {
        ...
        if (!name || data.entries.length >= 2) return;
      ...
      function paint() {
        ...
        data.entries.forEach((entry, index) => {
          ...
          remove.addEventListener("click", () => { data.entries.splice(index, 1); paint(); onChange(); }); row.append(remove); rows.append(row);
        });
        root.append(rows);
        ...
        for (const [name, label, action] of [["Add", "+ Ajouter un LoRA", add], ["Swap", "Inverser l’ordre", () => { data.entries.reverse(); paint(); onChange(); }], ["Refresh", "Actualiser la liste", refresh]]) {
          ...
          button.addEventListener("click", action); if (name === "Add") button.hidden = data.entries.length === 2; actions.append(button);
        }
        if (!perPass()) {
      ...
      return {
        ...
        restore(value) { if (config.supported && value) { data = copy(value); paint(); } },
        restoreAttempt(attempt) {
          if (!config.supported) return;
          data = attempt.video_loras ? copy(attempt.video_loras) : {
            version: "0.1.0", enabled: Boolean(attempt.video_lora), clip_last_layer: perPass() ? null : attempt.video_lora?.clip_last_layer ?? -2,
