...
from .h3_phase_plan import canonical_h3_phase_plan, lint_h3_phase_plan, h3_phase_plan_warnings
from .direct_ref2v_plan import (
    ...
    lint_direct_ref2v_action_plan_v4_late_anchor,
...
_FL2VA_DIRECT_V4_CONTRACT = "minimax.h3.fl2va.direct_compact_h3_v4"
_FL2VA_DIRECT_V5_CONTRACT = "minimax.h3.fl2va.direct_compact_h3_v5"
_FL2VA_ANIMAL_INTERVIEW_V1_CONTRACT = (
    "minimax.h3.fl2va.animal_interview_compact_h3_v1"
)
_FL2VA_ANIMAL_INTERVIEW_V2_CONTRACT = (
    "minimax.h3.fl2va.animal_interview_compact_h3_v2"
)
_FL2VA_ANIMAL_INTERVIEW_CONTRACTS = {
    _FL2VA_ANIMAL_INTERVIEW_V1_CONTRACT,
    _FL2VA_ANIMAL_INTERVIEW_V2_CONTRACT,
}
_REF2V_DIRECT_V4_CONTRACT = "minimax.h3.ref2v.direct_supervised_h3_v4"
_FL2VA_MOTION_CONTRACTS = {
    _FL2VA_DIRECT_V3_CONTRACT,
    _FL2VA_DIRECT_V4_CONTRACT,
    _FL2VA_DIRECT_V5_CONTRACT,
    *_FL2VA_ANIMAL_INTERVIEW_CONTRACTS,
...
_FL2VA_DIRECT_MULTISHOT_CONTRACTS = {
    _FL2VA_DIRECT_MULTISHOT_CONTRACT, MULTISHOT_PLAN_CONTRACT, MULTISHOT_DIRECT_CONTRACT,
}
...
_FL2VA_DIRECT_EDITABLE_CONTRACT = RevisedDocumentContract(
    "H3 Base editable document",
    tuple(f"{field}:" for field in _I2VA_FIELDS),
)
...
class PromptCompositionService:
    ...
    def edit(
        ...
        if (
            cookbook.output_contract in _FL2VA_DIRECT_MULTISHOT_CONTRACTS
            and stage is CompositionStage.BEAT_SHEET
        ):
            validate_direct_fl2va_multishot_dialogues(
                content,
        ...
        *,
        ...
        if instruction is None:
            ...
            if (
                ...
                prefix = _direct_fl2va_multishot_compiler_context(
                    session,
                    composition,
                    cookbook,
                )
                context = decode_direct_fl2va_multishot_context(
                    current.compiler_context or ""
                )
                editable_current = rehydrate_direct_fl2va_multishot_document(
                    current.content,
                    context,
                )
            if (
                cookbook.output_contract in _FL2VA_DIRECT_CONTRACTS
                and stage is CompositionStage.FINAL_PROMPT
            ):
                prefix = _direct_fl2va_compiler_context(
                    session,
                    composition,
                    cookbook,
                )
                context = decode_direct_fl2va_context(
                    current.compiler_context or ""
                )
                editable_current = rehydrate_direct_fl2va_document(
                    current.content,
                    context,
                )
            if (
                cookbook.output_contract in _CAMERA_OWNED_MONO_CONTRACTS
                and cookbook.output_contract not in _FL2VA_DIRECT_CONTRACTS
                and stage is CompositionStage.FINAL_PROMPT
            ):
                if cookbook.output_contract in {
                    _REF2V_DIRECT_V3_CONTRACT,
            ...
            if cookbook.output_contract == MULTISHOT_DIRECT_CONTRACT:
                revision_values["SEQUENCE"] = prefix[len(FL2VA_MULTISHOT_CONTEXT_MARKER):]
            if (
                cookbook.output_contract in _PLANNED_CONTRACTS
                and stage is CompositionStage.FINAL_PROMPT
            ):
                ...
                if cookbook.output_contract in _FL2VA_BASE_CONTRACTS:
                    revision_values["PLAN"] = _remove_h3_base_source_filenames(
                        revision_values["PLAN"],
                        session,
                    )
                if cookbook.output_contract in _DIRECT_MULTIMODAL_CONTRACTS:
        ...
        if (
            ...
            prefix = _direct_fl2va_compiler_context(session, composition, cookbook)
        if (
            stage is CompositionStage.FINAL_PROMPT
            and cookbook.output_contract in _FL2VA_DIRECT_MULTISHOT_CONTRACTS
            and (cookbook.output_contract != MULTISHOT_DIRECT_CONTRACT or instruction is not None)
        ):
            prefix = _direct_fl2va_multishot_compiler_context(
                session,
                composition,
        ...
        request = CompletionRequest(
            ...
            images=(
                ...
                or (
                    cookbook.output_contract in {_SUPER_FAST_REF2V_DIRECT_CONTRACT, DIRECT_PROMPT_CONTRACT, MULTISHOT_DIRECT_CONTRACT}
                    and stage is CompositionStage.FINAL_PROMPT
                )
                else ()
            ),
    ...
    def _direct_reference_images(
        ...
        composition: PromptComposition,
        *,
        include_source_filenames: bool = True,
    ) -> tuple[ImageInput, ...]:
    ...
    def _generation_prompts(
        ...
        brief = preparation_source(session, composition)
        if cookbook.output_contract in {DIRECT_PROMPT_CONTRACT, MULTISHOT_DIRECT_CONTRACT}:
        ...
        if cookbook.output_contract in _DIRECT_MULTIMODAL_CONTRACTS:
            if stage is CompositionStage.BEAT_SHEET:
                ...
                compact_plan = (
                    compact_base
                    or cookbook.output_contract == _REF2V_DIRECT_V4_CONTRACT
                )
                return (
                    _required_prompt(
            ...
            return (
                ...
                _render(
                    ...
                    BRIEF=("" if cookbook.output_contract in _FL2VA_BASE_CONTRACTS else brief.content),
                    PLAN=(
                        _remove_h3_base_source_filenames(
                            _writer_action_plan(cookbook, action_plan.content),
                            session,
                        )
                        if cookbook.output_contract in _FL2VA_BASE_CONTRACTS
                        else _writer_action_plan(cookbook, action_plan.content)
                    ),
                    REFERENCE_MAPPING=(
                        direct_h3_base_reference_mapping(
                            session,
    ...
    def _append_revision(
        ...
        *,
        ...
        if cookbook.output_contract == DIRECT_PROMPT_CONTRACT:
            ...
            session = self.sessions.get(composition.source_session_id)
            validate_compact_multishot(content, compiler_context, _mono_direct_context(session, composition, cookbook))
            active = composition.document(stage).active_revision
            if origin in {RevisionOrigin.MANUAL, RevisionOrigin.REWRITE} and active is not None:
                if compiler_context != active.compiler_context:
                    raise ValueError("La révision doit conserver le découpage courant ; régénérez pour le changer.")
        if (
            cookbook.output_contract in _FL2VA_DIRECT_MULTISHOT_CONTRACTS
            and stage is CompositionStage.BEAT_SHEET
        ):
            ...
            if cookbook.output_contract == MULTISHOT_PLAN_CONTRACT:
                requested_duration_ms = requested_duration_ms or 8000
            if requested_duration_ms is not None:
                content = _align_base_multishot_duration(
                    cookbook,
                    content,
        ...
        if (
            ...
            if cookbook.output_contract in _DIRECT_DIALOGUE_CONTRACTS:
                if cookbook.output_contract in _FL2VA_ANIMAL_INTERVIEW_CONTRACTS:
                    content, _ = compile_animal_interview_dialogue_cues(
                        content,
                        action_plan.content,
                        juvenile_animal_voice=(
                            cookbook.output_contract
                            == _FL2VA_ANIMAL_INTERVIEW_V2_CONTRACT
                        ),
                    )
        ...
        if stage is CompositionStage.FINAL_PROMPT and cookbook.output_contract in {
            ...
            *_REF2V_DIRECT_MULTISHOT_CONTRACTS,
            ...
            if cookbook.output_contract in _REF2V_DIRECT_MULTISHOT_CONTRACTS:
            ...
            elif cookbook.output_contract in _FL2VA_DIRECT_MULTISHOT_CONTRACTS:
                if cookbook.output_contract != MULTISHOT_DIRECT_CONTRACT:
                    expected_context = _direct_fl2va_multishot_compiler_context(
                        self.sessions.get(composition.source_session_id), composition, cookbook,
                    )
                    if compiler_context != expected_context:
                        raise ValueError("H3 Base multi-shot compiler context must come from the approved plan")
                context = decode_direct_fl2va_multishot_context(compiler_context)
                directives = context.directives
                mode = H3ProtocolMode(context.mode.value)
                rehydrate_direct_fl2va_multishot_document(content, context)
                errors = lint_direct_fl2va_multishot_prompt(content, context)
                if errors:
                    raise ValueError(" ".join(errors))
            elif cookbook.output_contract in _FL2VA_DIRECT_CONTRACTS:
                expected_context = _direct_fl2va_compiler_context(
                    self.sessions.get(composition.source_session_id),
                    composition,
                    cookbook,
                )
                if compiler_context != expected_context:
                    raise ValueError(
                        "H3 Base compiler context must come from the approved plan"
                    )
        ...
        elif cookbook.output_contract in _FL2VA_BASE_CONTRACTS:
            _raise_fl2va_labels(
                self.sessions.get(composition.source_session_id),
                composition,
                stage,
        ...
        *,
        ...
        for event in self.gateway.stream(request):
            if event.kind is StreamEventKind.COMPLETED:
                ...
                terminal = True
                visible_prefix = "" if (cookbook.output_contract in {DIRECT_PROMPT_CONTRACT, MULTISHOT_DIRECT_CONTRACT}
                                          or _is_hidden_compiler_context(prefix)) else prefix
...
def lint_cookbook_document(
    ...
    if cookbook.output_contract in _FL2VA_DIRECT_MULTISHOT_CONTRACTS:
        if stage is CompositionStage.BEAT_SHEET:
            if cookbook.output_contract == MULTISHOT_DIRECT_CONTRACT:
                return (f"Stage {stage.value} does not belong to this cookbook.",)
            return lint_direct_fl2va_multishot_plan(content)
        if stage is CompositionStage.FINAL_PROMPT:
            return lint_direct_fl2va_multishot_prompt(content)
        return (f"Stage {stage.value} does not belong to this cookbook.",)
    if cookbook.output_contract in _FL2VA_DIRECT_CONTRACTS:
        if stage is CompositionStage.BEAT_SHEET:
            if cookbook.output_contract in _FL2VA_ANIMAL_INTERVIEW_CONTRACTS:
                return lint_animal_interview_action_plan(content)
            if cookbook.output_contract == _FL2VA_DIRECT_V4_CONTRACT:
                return lint_direct_ref2v_action_plan_v4_late_anchor(content)
            if cookbook.output_contract == _FL2VA_DIRECT_V5_CONTRACT:
                return lint_h3_phase_plan(content)
            return (
...
def composition_document_warnings(
    ...
    *,
    ...
    if cookbook.output_contract in _FL2VA_DIRECT_MULTISHOT_CONTRACTS:
        if stage is CompositionStage.BEAT_SHEET:
            return (multishot_state_warnings(content) if cookbook.output_contract == MULTISHOT_PLAN_CONTRACT
                    else direct_fl2va_multishot_plan_warnings(content))
        if stage is CompositionStage.FINAL_PROMPT:
            return _h3_protocol_warnings(H3ProtocolMode.FL2VA, content)
        return ()
    if cookbook.output_contract in _FL2VA_DIRECT_CONTRACTS:
        if stage is CompositionStage.BEAT_SHEET:
            if cookbook.output_contract == _FL2VA_DIRECT_V5_CONTRACT:
                return h3_phase_plan_warnings(content)
            return (
    ...
    if cookbook.output_contract in _REF2V_DIRECT_CONTRACTS:
        if stage is CompositionStage.BEAT_SHEET:
            if cookbook.output_contract == _REF2V_DIRECT_V4_CONTRACT:
                return direct_ref2v_action_plan_warnings_v4(content)
            return (
...
def _raise_fl2va_labels(
    ...
    mapping = composition_picture_mapping(composition)
    derive_h3_base_input_mode(session, mapping)
    if stage is CompositionStage.BEAT_SHEET:
        allowed = set(range(1, len(mapping) + 1))
        picture_numbers = {
            int(value) for value in re.findall(r"<Picture\s+(\d+)>", content)
        }
        if not picture_numbers.issubset(allowed):
            raise ValueError("H3 Base plan contains an unbound Picture label")
        return
    if stage is not CompositionStage.FINAL_PROMPT:
        raise ValueError("H3 Base exposes only beat_sheet and final_prompt")
    lowered = content.casefold()
    leaked = [
        reference.label
        for reference in session.references
        if reference.label and reference.label.casefold() in lowered
    ]
    if leaked:
        raise ValueError(
            "H3 Base final prompt must not contain source filename(s): "
            + ", ".join(leaked)
        )
...
def _validate_bindings(
    ...
    direct_target = cookbook.target_mode in {
        "ref2v_direct",
        "i2v_direct",
        "fl2va_direct",
    }
    if session.session_mode is PromptSessionMode.H3_BASE:
        if cookbook.target_mode != "fl2va_direct":
            raise ValueError("H3 Base sessions require the H3 Base direct cookbook")
    elif cookbook.target_mode == "fl2va_direct":
        raise ValueError("the H3 Base direct cookbook requires an H3 Base session")
    ...
    if cookbook.target_mode in {"i2v_direct", "fl2va_direct"} and set(
        all_reference_ids
    ) != {
        ...
        return encode_timed_camera_context(
            TimedCameraContext(
                ...
                placements=_direct_timed_camera_placements(
                    plan.content,
                    dialogue_aware=(
                        cookbook.output_contract == _REF2V_DIRECT_V4_CONTRACT
                    ),
                    motion_aware=(
                        cookbook.output_contract == _REF2V_DIRECT_V4_CONTRACT
                    ),
...
def _direct_fl2va_compiler_context(
    session: PromptLabSession,
    composition: PromptComposition,
    cookbook: PromptCookbookPort,
) -> str:
    if cookbook.output_contract not in _FL2VA_DIRECT_CONTRACTS:
        raise ValueError("this cookbook does not use the H3 Base compiler")
    ...
    motion_aware = cookbook.output_contract in _FL2VA_MOTION_CONTRACTS
    if motion_aware:
    ...
    return encode_direct_fl2va_context(
        DirectFL2VAContext(
            mode=mode,
            header=compile_h3_base_header(mode, plan.duration_ms),
...
def _direct_fl2va_multishot_compiler_context(
    session: PromptLabSession,
    composition: PromptComposition,
    cookbook: PromptCookbookPort,
) -> str:
    if cookbook.output_contract not in _FL2VA_DIRECT_MULTISHOT_CONTRACTS:
        raise ValueError("this cookbook does not use the H3 Base multi-shot compiler")
    if cookbook.output_contract == MULTISHOT_DIRECT_CONTRACT:
        current = composition.document(CompositionStage.FINAL_PROMPT).active_revision
        if current is None or current.compiler_context is None:
            raise ValueError("Générez le prompt multi-plan avant de le réviser.")
        return current.compiler_context
    plan_revision = _approved_stage(
        composition,
        CompositionStage.BEAT_SHEET,
        (
    ...
    plan = parse_direct_fl2va_multishot_plan(plan_revision.content)
    mode = derive_h3_base_input_mode(
        session,
        composition_picture_mapping(composition),
    )
    cameras = tuple(
        (
            replace(
                ...
                target_clause=normalize_h3_base_model_labels(
                    shot.camera.target_clause or "",
                    mode,
                ),
            )
    ...
    return encode_direct_fl2va_multishot_context(
        DirectFL2VAMultiShotCompilerContext(
            mode=mode,
            shot_starts_ms=plan.shot_starts_ms,
            shot_cameras=cameras,
            opening_compositions=tuple(
                normalize_h3_base_model_labels(shot.opening_composition, mode)
                for shot in plan.shots
            ),
            final_state_description=normalize_h3_base_model_labels(
            ...
            cut_policy="new_view" if cookbook.output_contract == _FL2VA_DIRECT_MULTISHOT_CONTRACT else "neutral",
        )
    )
...
def _compact_brief(content: str) -> str:
    """Remove formatting slack without discarding any approved Brief statement."""
...
def _compact_h3_base_brief(
    content: str,
    session: PromptLabSession,
) -> str:
    """Project the compact Brief without exposing local source filenames."""

    return _remove_h3_base_source_filenames(_compact_brief(content), session)


def _remove_h3_base_source_filenames(
    content: str,
    session: PromptLabSession,
) -> str:
    value = content
    replacements = {
        "first_frame": "the supplied first frame",
        "last_frame": "the supplied last frame",
    }
    for reference in session.references:
        if not reference.label:
            continue
        value = re.sub(
            re.escape(reference.label),
            replacements.get(reference.role, "the supplied frame"),
            value,
            flags=re.IGNORECASE,
        )
    return value
...
def _compact_json_schema(content: str, *, prune_metadata: bool = False) -> str:
    try:
        value = json.loads(content)
    except json.JSONDecodeError as error:
        raise ValueError("direct action-plan schema is not valid JSON") from error
    if prune_metadata:
        value = _prune_json_schema_metadata(value)
    return json.dumps(value, ensure_ascii=False, separators=(",", ":"))


def _prune_json_schema_metadata(value, *, property_map: bool = False):
    if isinstance(value, dict):
        return {
            key: _prune_json_schema_metadata(
                item,
                property_map=(key == "properties"),
            )
            for key, item in value.items()
            if property_map
            or key not in {"title", "description", "default", "examples"}
        }
    if isinstance(value, list):
        return [_prune_json_schema_metadata(item) for item in value]
    return value
...
def _align_base_multishot_duration(cookbook, content: str, duration_ms: int) -> str:
    align = (align_state_multishot_duration if cookbook.output_contract == MULTISHOT_PLAN_CONTRACT
             else align_direct_fl2va_multishot_duration)
    return align(content, duration_ms)
...
def _compile_content_with_context(
    ...
    *,
    ...
    if cookbook.output_contract == MULTISHOT_DIRECT_CONTRACT:
        ...
        if prefix.startswith(FL2VA_MULTISHOT_CONTEXT_MARKER):
            return compile_direct_fl2va_multishot_document(result, prefix), prefix
        return compile_compact_multishot(result, prefix)
    if cookbook.output_contract == DIRECT_PROMPT_CONTRACT:
        if stage is not CompositionStage.FINAL_PROMPT:
    ...
    content = _compile_content(
        ...
        dialogue_source_text=dialogue_source_text,
    )
    if (
        stage is CompositionStage.BEAT_SHEET
        and cookbook.output_contract in {
            *_FL2VA_DIRECT_CONTRACTS,
            _REF2V_DIRECT_V4_CONTRACT,
        }
        and source_text is not None
    ):
        requested_duration_ms = requested_h3_base_duration_ms(source_text)
        if requested_duration_ms is not None:
            content = align_direct_ref2v_action_plan_v2_duration(
                content,
                requested_duration_ms,
    ...
    if (
        ...
        decode_direct_fl2va_multishot_context(prefix)
        compiler_context = prefix
    elif (
        stage is CompositionStage.FINAL_PROMPT
        and cookbook.output_contract in _FL2VA_DIRECT_CONTRACTS
    ):
        decode_direct_fl2va_context(prefix)
        compiler_context = prefix
    elif (
        stage is CompositionStage.FINAL_PROMPT
        and cookbook.output_contract == _REF2V_SUPERVISED_CANONICAL_CONTRACT
...
def _compile_content(
    ...
    *,
    ...
    if (
        ...
        context = decode_direct_fl2va_multishot_context(prefix)
        content = normalize_dialogue_language_tags(
            compile_direct_fl2va_multishot_document(
        ...
        if cookbook.output_contract in {
            ...
            *_FL2VA_DIRECT_MULTISHOT_CONTRACTS,
            ...
            recovery_options["expected_dialogues"] = extract_explicit_dialogues(
                dialogue_source_text if dialogue_source_text is not None else source_text or ""
            )
    ...
    if not (
        stage is CompositionStage.FINAL_PROMPT
        and cookbook.output_contract == _REF2V_DIRECT_V4_CONTRACT
    ):
        _raise_lint(cookbook, stage, content)
    return content
...
def _writer_action_plan(cookbook: PromptCookbookPort, content: str) -> str:
    if cookbook.output_contract in _FL2VA_DIRECT_MULTISHOT_CONTRACTS:
        return direct_fl2va_multishot_writer_plan(content)
    if cookbook.output_contract in _REF2V_DIRECT_MULTISHOT_CONTRACTS:
        return (
...
def _direct_action_plan_schema(cookbook: PromptCookbookPort) -> str:
    if cookbook.output_contract in _FL2VA_DIRECT_MULTISHOT_CONTRACTS:
        return direct_fl2va_multishot_plan_schema()
    if cookbook.output_contract in _REF2V_DIRECT_MULTISHOT_CONTRACTS:
        return (
            direct_ref2v_multishot_plan_schema_v2()
            if cookbook.output_contract == _REF2V_DIRECT_MULTISHOT_V2_CONTRACT
...
def _direct_action_plan_canonicalizer(cookbook: PromptCookbookPort):
    if cookbook.output_contract == _FL2VA_DIRECT_V5_CONTRACT:
        return canonical_h3_phase_plan
    if cookbook.output_contract in _FL2VA_DIRECT_MULTISHOT_CONTRACTS:
        return canonical_direct_fl2va_multishot_plan
    if cookbook.output_contract in _REF2V_DIRECT_MULTISHOT_CONTRACTS:
        return (
            canonical_direct_ref2v_multishot_plan_v2
            if cookbook.output_contract == _REF2V_DIRECT_MULTISHOT_V2_CONTRACT
    ...
    if cookbook.output_contract in {
        ...
        return canonical_direct_ref2v_action_plan_v4_late_anchor
    if cookbook.output_contract in _FL2VA_MOTION_CONTRACTS:
...
def _parse_direct_arbitrable_plan(
    ...
    if cookbook.output_contract in _FL2VA_DIRECT_MULTISHOT_CONTRACTS:
        return parse_direct_fl2va_multishot_plan(content)
    if cookbook.output_contract in _REF2V_DIRECT_MULTISHOT_CONTRACTS:
        return (
...
def _revision_document_contract(
    ...
    *,
    ...
    if (
        ...
        if compiler_context is None:
            raise ValueError("H3 Base multi-shot revision is missing compiler context")
        context = decode_direct_fl2va_multishot_context(compiler_context)
        return direct_fl2va_multishot_editable_contract(context.shot_count)
    if (
        cookbook.output_contract in _FL2VA_DIRECT_CONTRACTS
        and stage is CompositionStage.FINAL_PROMPT
    ):
        return _FL2VA_DIRECT_EDITABLE_CONTRACT
    if (
...
def _stage_contract(
    ...
    *,
    ...
    if (
        ...
        markers = ", ".join(
            f"{field}:"
            for field in direct_fl2va_multishot_editable_fields(context.shot_count)
        )
        return (
            f"Return exactly {markers}, once each and in that order. Output no "
            "mode header, image label, shot heading, cut phrase, timestamp, camera "
            "prose or final-state prose. Preserve every dialogue placeholder. "
            f"PanelForge compiles the exact {context.shot_count} shots from the "
            + ("fixed sequence." if cookbook.output_contract == MULTISHOT_DIRECT_CONTRACT else "approved Plan.")
        )
    if (
        cookbook.output_contract in _FL2VA_DIRECT_DIALOGUE_CONTRACTS
        and stage is CompositionStage.FINAL_PROMPT
        ...
        return (
            ...
            "overall_soundscape:, and non_diegetic_music:. Output no mode header, "
            "image label, local filename, camera prose, cut, or [Shot 2]. Preserve "
            "every PLAN camera landmark and exact dialogue cue; PanelForge owns "
            "the duration, final landmark, dialogue clauses and camera clauses."
        )
    if (
        cookbook.output_contract == _FL2VA_DIRECT_CONTRACT
        and stage is CompositionStage.FINAL_PROMPT
    ):
        return (
            "Return exactly integrated_multimodal_description:, "
            "overall_soundscape:, and non_diegetic_music:. Output no mode header, "
            "Picture/Image/Subject label, camera placeholder, directive ID, "
            "camera-motion prose, cut, or [Shot 2]. Preserve every positive "
            "PLAN.camera_landmarks_ms timestamp; PanelForge reinserts the exact "
            "T2VA/I2VA/L2VA/FL2VA header and canonical camera clauses."
        )
    if (
        cookbook.output_contract == _SUPER_FAST_REF2V_DIRECT_CONTRACT
        and stage is CompositionStage.FINAL_PROMPT
