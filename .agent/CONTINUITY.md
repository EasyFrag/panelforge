...
- **Contrainte confirmée par l’utilisateur le 2026-09-10, à préserver** : Combat et Classique seront deux familles de préparation indépendantes. Une modification spécifique ne change pas l’autre famille. Partager les améliorations générales via un socle à versions exactes, avec adoption explicite examinée pour les deux familles dans le même patch ; aucun héritage de « Classique latest ». Conserver les familles/versions jusque dans les révisions conversationnelles. Application/file/rendu restent communs. L’utilisateur demande expressément de garder cette décision en mémoire ; implémentation Combat autorisée le 2026-09-10 et livrée dans ce checkout, sans exécution des tests ni appels de génération.

## Current state

- **Classique expérimental autorisé, 2026-09-10 — point GitHub avant patch demandé**. L’utilisateur autorise l’implémentation de la proposition Classique Mise en scène en deux appels (H3/REF2V, Auto ou 1–6 plans), à condition de publier d’abord une version GitHub de l’état actuel. Préparation du snapshot des patches précédents sur `h3-video-lora` dans `D:\Code\localQ\.panelpatch` ; dépôt GitHub retrouvé via le checkout principal : `EasyFrag/panelforge`. Aucun code du nouveau patch n’est modifié avant cette publication. Anciennes recettes/défauts et Combat à conserver ; analyse média et Sensualité restent en backlog. Tests à préparer seulement, aucune génération/LLM/service à lancer.

- **Classique expérimental en deux étapes : discussion active, 2026-09-10**. L’utilisateur met en attente les deux chantiers analyse vidéo/images et Sensualité / jeu d’acteur non explicite, puis demande si l’approche Combat à deux appels mérite une version expérimentale Classique et si le nombre de plans devrait rester dans l’intention. Manifests relus : Classique propose déjà H3 mono planned 1.2.0 / REF2V mono planned 1.1.0 / H3 multi planned 1.1.0 en deux appels. Proposition : adapter surtout le Plan de mise en scène et la transmission au rédacteur de Combat 1.3, avec politiques/exemples Classique propres et socle neutre à versions exactes ; ne pas attribuer le gain au seul nombre d’appels. Entrée expérimentale distincte, deux appels initiaux seulement, anciens 1/2/3 et défauts conservés. Petit contrôle nombre de plans Auto / 1–6 proposé : Auto suit l’intention explicite sinon laisse le Plan choisir ; sélection manuelle prioritaire et visible. Actions et coupes indépendantes ; scène calme possible, pas d’intensité Combat imposée. Documentation `docs/backlog.md`. **Discussion seulement, aucun patch fonctionnel autorisé**, aucun test/LLM/rendu/service lancé. L’entrée suivante décrit la discussion précédente, désormais remise en backlog.
...
- **Implémenter Classique expérimental en deux appels, autorisé** : publier d’abord le snapshot GitHub demandé, puis ajouter la recette H3/REF2V et le contrôle Auto/1–6 plans. Préserver les recettes/défauts et l’isolation Combat/Classique, préparer les tests sans les lancer. Le nombre d’appels seul n’établit pas la qualité ; comparaison utilisateur à réglages constants avant un changement éventuel de défaut. Proposition dans `docs/backlog.md`.

- **Validation utilisateur des deux LoRA de rendu** : patch implémenté ; procédure/tests dans `docs/h3-multiple-loras-proposal.md`. Après ses traitements, l’utilisateur recharge le Lab et choisit BUNNY **0.1.2** pour Combat V2 → Motion Repair à **0,60/0,20 chacun**, valeurs désormais validées comme point de départ, sans prétendre être un optimum officiel. Comparer avec Motion seul désactivé, puis reprise/switch de recette dans H3 et REF2V ; tester les deux lignes facultatives du basique. Une reprise historique garde sa recette et son seul fichier. Aucun test, LLM, rendu ou service à lancer à sa place.
...
- **Deux chantiers conservés en backlog sur demande utilisateur** : analyse vidéo/images → prompt (P1, proposition Video Lab dans `docs/backlog.md`) et Sensualité / jeu d’acteur non explicite. Ne pas les démarrer pendant la discussion Classique expérimentale. Recherche de seeds toujours mise de côté.
