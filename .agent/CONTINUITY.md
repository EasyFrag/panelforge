...
- **Point de restauration avant BUNNY demandé, 2026-09-09** : l’utilisateur demande de figer l’état sur GitHub avant le patch et confirme l’exigence de conserver les générations actuelles H3 et REF2VA. Snapshot nommé **`snapshot-avant-bunny-h3-2026-09-09`**, publié sur une branche dédiée `snapshots/avant-bunny-h3-2026-09-09` du dépôt `https://github.com/EasyFrag/panelforge`, après vérification des références distantes. Cette note fait partie de l’état à figer ; l’existence distante doit être contrôlée après création du commit/tag, avant d’annoncer la sauvegarde faite. Périmètre : toutes les modifications de code, recettes, tests préparés et documentation depuis le tag pré-masque, notamment Edit/FireRed, H3 et DLSS ; aucune donnée workspace, média ou poids de modèle. **BUNNY restera une recette de rendu supplémentaire**, les recettes H3 Base 0.1.3 et REF2V 0.2.0 et leurs réglages resteront sélectionnables. Aucun test, appel LLM, rendu, installation ou service à lancer pour cette sauvegarde ; ancien tag `stable-avant-masque-2026-09-06` inchangé.
...
- **Avant tout patch BUNNY** : créer puis vérifier sur GitHub le commit et le tag `snapshot-avant-bunny-h3-2026-09-09` sur la branche dédiée `snapshots/avant-bunny-h3-2026-09-09`. Conserver explicitement les recettes de génération actuelles H3 / REF2VA dans le sélecteur du futur patch, comme confirmé par l’utilisateur. Ne pas remplacer le défaut actuel par BUNNY ni modifier les réglages historiques ; pas de test, rendu ou redémarrage pour la sauvegarde.

- **Sujet courant : alignement BUNNY fichier (3) sur Bucket**, décrit dans `docs/bunny-h3-integration-proposal.md`. Retenir routage du modèle par mode, 0,9 MP ×1,0, Turbo 9/4/5 ou sans Turbo 30/25/5, un LoRA à deux forces, preview live réutilisant notre infrastructure. Expliquer le rôle des trois paramètres steps et le caractère expérimental du profil sans Turbo. Conserver la préparation LLM et les ateliers ; rendre la recette propre à chaque essai et vérifier raccordement des références/frames et preview aux deux passes. Aucun rendu/test/installation ou service à lancer à sa place ; l’utilisateur demande encore un alignement, intégration pas encore autorisée.
